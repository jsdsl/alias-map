...
describe("Initialization", () => {
    test("Basic initialization.", () => {
        let aliasMap = new alias_map_1.AliasMap();
...
describe("Per-method tests.", () => {
    ...
    beforeEach(() => {
        aliasMap = new alias_map_1.AliasMap();
        populate();
    });
    describe("#get", () => {
        ...
        describe("Attempt to retrieve a value via it's 'secondary aliases' succeeds.", () => {
            ...
            ])("Alias set #%i.", (index, expectedValue, aliases) => {
                for (let alias of aliases)
                    expect(aliasMap.get(alias)).toBe(expectedValue);
            });
        });
    ...
    describe("#listAliases", () => {
        describe("Correct aliases are returned for a 'primary alias'.", () => {
            describe.each([
            ...
            ])("Alias set #%i.", (index, searchAlias, aliases) => {
                let resultSet;
                beforeAll(() => {
                ...
                test("Result contains the correct values.", () => {
                    for (let alias of aliases)
                        expect(resultSet).toContain(alias);
                });
    ...
    describe("#size", () => {
        test("Returns zero for newly initialized map.", () => {
            expect((new alias_map_1.AliasMap()).size()).toBe(0);
        });
